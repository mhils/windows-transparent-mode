...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ParseError::UnknownIpVersion => write!(f, "Unknown IP version"),
...
impl InternetPacket {
    pub fn new(data: Vec<u8>) -> Result<InternetPacket, ParseError> {
        let ip_version = match data[0] >> 4 {
            4 => IpVersion::V4,
            6 => IpVersion::V6,
            _ => return Err(ParseError::UnknownIpVersion)
        };
        ...
        let (transport_proto, transport_proto_offset) = match ip_version {
            ...
            IpVersion::V6 => {
                ...
                while IPV6_EXTENSION_HEADERS.contains(&next_header) {
                    ...
                    offset += data[offset + 1] as usize + 8;
                }
        ...
        assert!(transport_proto_offset + 4 <= data.len());

        Ok(InternetPacket { data, ip_version, transport_proto, transport_proto_offset })
    }
    ...
    pub fn connection_id(&self) -> ConnectionId {
        ConnectionId {
            ...
            src: SocketAddr::from((self.src_ip(), self.src_port())),
            dst: SocketAddr::from((self.dst_ip(), self.dst_port())),
        }
    }
    ...
    pub fn payload(&self) -> &[u8] {
        ...
        match self.transport_proto {
            TransportProtocol::TCP => {
                let data_offset = (self.data[self.transport_proto_offset + 12] >> 4) as usize;
                &self.data[self.transport_proto_offset + data_offset..]
            },
            TransportProtocol::UDP => {
                &self.data[self.transport_proto_offset + 8..]
            }
        }
    }
}
